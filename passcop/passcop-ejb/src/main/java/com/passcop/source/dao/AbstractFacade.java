...
public abstract class AbstractFacade<T> implements Serializable {
	...
	public void removeList(Collection<T> entities) {
		entities.stream().forEach(e -> getEntityManager().remove(getEntityManager().merge(e)));
	}

	// Metodos para control de predicados y ordenes

	public void clearWherePredicates() {
		setWherePredicates(new ArrayList<>());
	}

	public void clearOrderByPredicates() {
		setOrderByPredicates(new ArrayList<>());
	}
	...
	public boolean isTransient(T object) {
		if (object != null) {
			...
			if (objecjectID == null)
				...
				if (objecjectID.toString().equals("0"))
					return true;
				return false;
		}
		return true;
